...
const navItems = [
  ...
  ['Leadership', '/leadership'], ['Beyond Tech', '/beyond-tech'], ['About', '/#about'], ['Contact', '/#contact'],
];
    ...
    <section className="hero" id="home">
      ...
      <div className="hero-copy">
        ...
        <MotionDiv className="hero-actions" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .75 }}>
          ...
          <Link className="button button-ghost" to="/#about">Meet me</Link>
...
function ProjectVisual({ type }) {
  if (type === 'clinical') return <div className="project-visual clinical-visual"><div className="pulse-line"><i/><i/><i/><i/><i/><i/></div><div className="risk-score"><span>model signal</span><strong>91.6</strong><small>ROC–AUC × 100</small></div><div className="clinical-tags"><span>SHAP</span><span>40 TESTS</span><span>9,105 PATIENTS</span></div></div>;
...
function ResearchDetail() {
  ...
  return <main className="detail-page research-detail"><section className="detail-hero dark"><Link to="/research" className="back-link">← Research archive</Link><div className="detail-title"><p>{pub.number} · {pub.type} · {pub.year}</p><h1>{pub.title}</h1><p>{pub.summary}</p></div><DetailMetric value={pub.highlight} label={pub.label}/></section><section className="research-body"><div><p className="eyebrow">Citation</p><p>{pub.citation}</p><div className="tags">{pub.technologies.map(x=><span key={x}>{x}</span>)}</div></div><div><CaseBlock title="My contribution" text={pub.contribution}/><CaseBlock title="Methodology" text={pub.methodology}/><CaseBlock title="Key findings" text={pub.findings}/><div className="card-actions">{pub.publicationUrl&&<ArrowLink href={pub.publicationUrl}>Read publication</ArrowLink>}{pub.projectUrl&&<ArrowLink href={pub.projectUrl} secondary cursor="CODE">Related implementation</ArrowLink>}</div></div></section><ContactSection/></main>;
}
